 ...
 *
 * The overlay is rendered into `document.body` through a portal. The header carries
 * `backdrop-blur`, and a backdrop filter makes an element the containing block for
 * its `position: fixed` descendants — so an overlay rendered in place is clamped to
 * the header's own ~72px box instead of filling the viewport, leaving the drawer a
 * clipped sliver with its navigation cut off. The portal moves it out of that
 * subtree, which is the only reliable fix short of dropping the blur.
 */
export function MobileMenu({
    ...
    <div className="fixed inset-0 z-100 lg:hidden">
      ...
      >
        ...
        <div className="flex flex-col gap-3 border-t border-navy-light p-4">
          ...
          <Button as={Link} href="/trips" variant="gold" fullWidth onClick={() => setOpen(false)}>
            {tNav('bookNow')}
          </Button>
        </div>
      </div>
  ...
  return (
    <>
      ...
      {/*
        `open` starts false and can only be set by a click, so this branch never runs
        during server rendering or hydration — `document` is guaranteed to exist by
        the time the portal is created.
      */}
